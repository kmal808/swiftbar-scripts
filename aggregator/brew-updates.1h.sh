...
/opt/Hombrew/bin update > /dev/null || exit_with_error;

PINNED=$(/opt/Hombrew/bin list --pinned);
OUTDATED=$(/opt/Hombrew/bin outdated --quiet);
...
echo "---";
if [ -n "$UPDATES" ]; then
  echo "Upgrade all | bash=/opt/Hombrew/bin param1=upgrade terminal=false refresh=true"
  echo "$UPDATES" | awk '{print $0 " | terminal=false refresh=true bash=/opt/Hombrew/bin param1=upgrade param2=" $1}'
fi
